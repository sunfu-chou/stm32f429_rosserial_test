#ifndef _ROS_nlink_parser_LinktrackAnchorframe0_h
#define _ROS_nlink_parser_LinktrackAnchorframe0_h
...
namespace nlink_parser
{
  ...
  class LinktrackAnchorframe0 : public ros::Msg
  {
    public:
      ...
      typedef nlink_parser::LinktrackTag _nodes_type;
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(nodes_lengthT > nodes_length)
        this->nodes = (nlink_parser::LinktrackTag*)realloc(this->nodes, nodes_lengthT * sizeof(nlink_parser::LinktrackTag));
      nodes_length = nodes_lengthT;
      for( uint32_t i = 0; i < nodes_length; i++){
      offset += this->st_nodes.deserialize(inbuffer + offset);
        memcpy( &(this->nodes[i]), &(this->st_nodes), sizeof(nlink_parser::LinktrackTag));
      }
     return offset;
    }

    virtual const char * getType() override { return "nlink_parser/LinktrackAnchorframe0"; };
    virtual const char * getMD5() override { return "cbcf3e0d7d7505d0e2909378d4beb301"; };
